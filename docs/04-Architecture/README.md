...
- **Cloudways Hybrid Stack (repo in `public_html`)**: Use root [`.htaccess`](../../.htaccess) — maps URLs into **`client/dist/`**, proxies **`/api`** to Node, then SPA fallback. Run **`npm run build`** so **`client/dist/sitemap.xml`** exists. If you deploy only **`client/dist/`** contents into `public_html`, use [`client/public/.htaccess`](../../client/public/.htaccess) instead.
