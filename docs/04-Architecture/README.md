...
- **Sitemap (build-time only)**: **`npm run build`** runs **`buildSitemap.ts`**, writing **`client/dist/sitemap.xml`** and **`server/dist/sitemap.xml`** from MongoDB using **`PUBLIC_SITE_URL`**. No live generation at request time.
- **Cloudways Hybrid Stack**: Deploy **`client/dist/`** to **`public_html`**. Apache uses [`client/public/.htaccess`](../../client/public/.htaccess) — static **`/sitemap.xml`**, proxy **`/api`** to Node on port 3001, then SPA fallback to **`index.html`**. Do not add nginx rules for the sitemap; Hybrid Stack is **`.htaccess`-driven**.
