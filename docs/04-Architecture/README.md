...
- **Cloudways Hybrid Stack**: Deploy **`client/dist/`** to **`public_html`**. Apache uses [`client/public/.htaccess`](../../client/public/.htaccess) — proxy **`/api`** to Node on port 3001, then SPA fallback. **`/sitemap.xml`** and **`/robots.txt`** are ordinary static files.
