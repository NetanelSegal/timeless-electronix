...
- **Cloudways/Apache SPA rewrite order**: if serving the client with Apache, keep exclusions for **`/api/*`** and **`/sitemap.xml`** before the SPA fallback rule. The repository includes [`client/public/.htaccess`](../../client/public/.htaccess) for this ordering.
