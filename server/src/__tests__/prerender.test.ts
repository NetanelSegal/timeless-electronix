...
beforeEach(() => {
  ...
  catalogDir = path.join(tmp, "catalog");
...
describe("prerenderProductShells", () => {
  it("writes one shell per valid slug", async () => {
    ...
    expect(fs.readFileSync(path.join(catalogDir, "abc-123.html"), "utf8")).toBe(SHELL);
    expect(
      fs.readFileSync(path.join(catalogDir, "3m-3341-1s-nb889-20.html"), "utf8"),
    ).toBe(SHELL);
  });

  it("writes the marker only after the whole set is emitted", async () => {
    await prerenderProductShells({ catalogDir, shellPath, slugs: ["abc-123"] });
    expect(fs.existsSync(path.join(catalogDir, PRERENDER_MARKER))).toBe(true);
  });
  ...
  it("refuses slugs that would escape the catalog directory", async () => {
    ...
    expect(fs.readdirSync(catalogDir).sort()).toEqual([
      PRERENDER_MARKER,
      "ok-1.html",
    ]);
  });
  ...
  it("refuses slugs outside the slug charset", async () => {
    ...
    expect(fs.readdirSync(catalogDir)).toEqual([PRERENDER_MARKER]);
  });
  ...
  it("replaces shells left by a previous build", async () => {
    ...
    fs.writeFileSync(path.join(catalogDir, "abc-123.html"), "STALE", "utf8");
